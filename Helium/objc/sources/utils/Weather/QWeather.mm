...
- (NSString *)conditionsEmoji {
    ...
    if (self.now && [self.now[@"code"] isEqualToString:@"200"]) {
        NSInteger weatherCode = getIntFromDictKey(self.now[@"now"], @"icon");
...
- (NSString *)humidity:(BOOL)withSymbol {
    ...
    if (self.now && [self.now[@"code"] isEqualToString:@"200"]) {
        humidityString = [NSString stringWithFormat:withSymbol ? @"%.0f%%" : @"%.0f", getDoubleFromDictKey(self.now[@"now"], @"humidity")];
    }
...
- (NSString *)precipitationPercentNextHour:(BOOL)withSymbol {
    ...
    if (self.hourly && [self.hourly[@"code"] isEqualToString:@"200"]) {
        ...
        if (hourlyForecasts != nil && hourlyForecasts.count > 0) {
            precipitationString = [NSString stringWithFormat:withSymbol ? @"%.0f%%" : @"%.0f", getDoubleFromDictKey([hourlyForecasts firstObject], @"pop")];
        }
    } else {
        precipitationString = @"--";
    }
        ...
        [self updateLocation:^(NSString *location) {
            dispatch_async(concurrentQueue, ^{
                               ...
                               if (location != nil && nowTime - weakSelf.lastUpdateTime > 60) {
                                   ...
                                   weakSelf.now = [weakSelf fetchNowWeatherForLocation:location != nil ?
                                                   location : weakSelf.lastLocation] ? : weakSelf.now;
                                   weakSelf.daily = [weakSelf fetchTodayWeatherForLocation:location != nil ?
                                                     location : weakSelf.lastLocation] ? : weakSelf.daily;
                                   weakSelf.hourly = [weakSelf fetch24HoursWeatherForLocation:location != nil ?
                                                      location : weakSelf.lastLocation] ? : weakSelf.hourly;
