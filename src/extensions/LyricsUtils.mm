...
#import "../extensions/LRCLyricsParser.h"
...
- (void)getLyric {
    ...
    if (self.gettingLyric == false && self.noLyric == -1) {
        ...
        [self getKugouLyricByName:[self trimTitle:self.title] artist:[self trimArtist:self.artist] album:self.album completion:^(NSString *lyric, NSError *error) {
            HMLog(lyric);
            if (error) {
                HMLog(error);
...
- (void)fallbackNetease {
    [self getNetEaseLyricByName:[self trimTitle:self.title] artist:[self trimArtist:self.artist] album:self.album completion:^(NSString *lyric, NSError *error) {
        HMLog(lyric);
        if (error) {
            HMLog(error);
            // [self fallbackKugou];
...
- (void)fallbackKugou {
    [self getKugouLyricByName:[self RomajiToHiragana:[self trimTitle:self.title]] artist:[self RomajiToHiragana:[self trimArtist:self.artist]] album:[self RomajiToHiragana:self.album] completion:^(NSString *lyric, NSError *error) {
        HMLog(lyric);
        if (error) {
            HMLog(error);
...
- (void)fallbackNetease2 {
    [self getNetEaseLyricByName:[self RomajiToHiragana:[self trimTitle:self.title]] artist:[self RomajiToHiragana:[self trimArtist:self.artist]] album:[self RomajiToHiragana:self.album] completion:^(NSString *lyric, NSError *error) {
        HMLog(lyric);
        if (error) {
            HMLog(error);
            self.noLyric = 1;
...
- (void)getSongIDForSong:(NSString *)title artist:(NSString *) artist album:(NSString *) album completion:(void (^)(NSNumber *songID, NSError *error))completion {
    ...
    [self postDataToURL:@"http://music.163.com/api/search/pc" postString:postString completion:^(NSString *data, NSError *error) {
        HMLog(data);
        if (error) {
        ...
        if (songs.count > 0) {
            ...
            for (NSDictionary *songItem in songs) {
                ...
                float titleConformRatio = MAX(MAX([self calculateSimilarityBetweenText:title andText:songName], 
                                            [self calculateSimilarityBetweenText:[self TCToSC:title] andText:[self TCToSC:songName]]),
                                            [self calculateSimilarityBetweenText:[self KanjoToRomaji:title] andText:[self KanjoToRomaji:songName]]);
                float artistConformRatio = MAX(MAX([self calculateSimilarityBetweenText:artist andText:singerName], 
                                            [self calculateSimilarityBetweenText:[self TCToSC:artist] andText:[self TCToSC:singerName]]),
                                            [self calculateSimilarityBetweenText:[self KanjoToRomaji:artist] andText:[self KanjoToRomaji:singerName]]);
                float albumConformRatio = MAX(MAX([self calculateSimilarityBetweenText:album andText:albumName],
                                            [self calculateSimilarityBetweenText:[self TCToSC:album] andText:[self TCToSC:albumName]]),
                                            [self calculateSimilarityBetweenText:[self KanjoToRomaji:album] andText:[self KanjoToRomaji:albumName]]);
                HMLog(titleConformRatio, artistConformRatio, albumConformRatio);
                
                float ratio = sqrtf(titleConformRatio * artistConformRatio);
                if (titleConformRatio > 0.8 && artistConformRatio == 0) ratio = albumConformRatio;
                HMLog(ratio);
            ...
            NSError *notFoundError = [NSError errorWithDomain:NSCocoaErrorDomain code:404 userInfo:@{NSLocalizedDescriptionKey: @"未找到匹配的歌曲"}];
...
- (void)downloadLyricForSongID:(NSNumber *)songID completion:(DataCompletionHandler)completionHandler {
    ...
    [self getDataFrom:lyricURLString completion:^(NSString *data, NSError *error) {
        if (error) {
...
- (void)getNetEaseLyricByName:(NSString *)title artist:(NSString *) artist album:(NSString *) album completion:(void (^)(NSString *lyric, NSError *error))completion {
    [self getSongIDForSong:title artist:artist album:album completion:^(NSNumber *songID, NSError *error) {
        if (error) {
            completion(nil, error);
        } else {
            [self downloadLyricForSongID:songID completion:^(NSString *responseData, NSError *error) {
                if (error) {
                    completion(nil, error);
                } else {
...
- (void)getHashForSong:(NSString *)title artist:(NSString *) artist album:(NSString *) album completion:(void (^)(NSString *songHash, NSError *error))completion {
    ...
    [self getDataFrom:url completion:^(NSString *data, NSError *error) {
        HMLog(data);
        if (error) {
            completion(nil, error);
            return;
        ...
        if (songs.count > 0) {
            ...
            for (NSDictionary *songItem in songs) {
                ...
                float ratio = sqrtf(titleConformRatio * artistConformRatio);
                if (titleConformRatio == 1 && artistConformRatio == 0) ratio = albumConformRatio;
                HMLog(ratio);
...
- (void)getSongByHash:(NSString *)hash completion:(void (^)(NSString *lyrics, NSError *error))completionHandler {
    ...
    [self getDataFrom:url completion:^(NSString *responseData, NSError *error) {
        HMLog(responseData);
        if (error) {
            completionHandler(nil, error);
            return;
        ...
        if (jsonError || !responseBody) {
            NSError *parsingError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSPropertyListReadCorruptError userInfo:@{NSLocalizedDescriptionKey: @"Error parsing JSON"}];
        ...
        if (status != 200) {
            NSError *statusError = [NSError errorWithDomain:NSURLErrorDomain code:status userInfo:nil];
        ...
        if (searchList.count <= 0) {
            NSError *noResultError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSURLErrorNoPermissionsToReadFile userInfo:@{NSLocalizedDescriptionKey: @"No search results found"}];
        ...
        [self getLyricsByID:ID accesskey:accesskey completion:completionHandler];
    }];
}
...
- (void)getLyricsByID:(NSString *)ID accesskey:(NSString *)accesskey completion:(void (^)(NSString *lyrics, NSError *error))completionHandler {
    ...
    [self getDataFrom:url completion:^(NSString *responseData, NSError *error) {
        ...
        if (lrcContent.length <= 0) {
            NSError *noContentError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSURLErrorResourceUnavailable userInfo:@{NSLocalizedDescriptionKey: @"No lyrics content found"}];
        ...
        if (!lyrics) {
            NSError *decodingError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSFileReadInapplicableStringEncodingError userInfo:@{NSLocalizedDescriptionKey: @"Error decoding lyrics content"}];
...
- (void)getKugouLyricByName:(NSString *)title artist:(NSString *)artist album:(NSString *) album completion:(void (^)(NSString *lyric, NSError *error))completion {
    [self getHashForSong:title artist:artist album:album completion:^(NSString *songHash, NSError *error) {
        if (error) {
            completion(nil, error);
        } else {
            [self getSongByHash:songHash completion:^(NSString *responseData, NSError *error) {
                if (error) {
                    completion(nil, error);
                } else {
...
- (void)getDataFrom:(NSString *)urlString completion:(DataCompletionHandler)completionHandler {
    ...
    self.currentTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if ([httpResponse statusCode] != 200) {
            ...
            NSError *statusError = [NSError errorWithDomain:NSURLErrorDomain code:[httpResponse statusCode] userInfo:nil];
            completionHandler(nil, statusError);
            return;
        }
