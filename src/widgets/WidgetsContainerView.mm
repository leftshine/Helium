...
- (void)setupView {
    ...
    [self addConstraints:@[
        [self.blurView.topAnchor constraintEqualToAnchor:self.topAnchor constant:-2],
        [self.blurView.leadingAnchor constraintEqualToAnchor:self.leadingAnchor constant:-4],
        [self.blurView.bottomAnchor constraintEqualToAnchor:self.bottomAnchor constant:2],
        [self.blurView.trailingAnchor constraintEqualToAnchor:self.trailingAnchor constant:4],
    ]];
}
...
- (void)reloadConfig {
    if (self.viewID) {
        ...
        // blur config
        NSDictionary *blurDetails = [config valueForKey:@"blurDetails"] ? [config valueForKey:@"blurDetails"] : @{@"hasBlur" : @(NO)};
...
- (void)reloadWidgets {
    if (self.viewID) {
        ...
        [[EZTimer shareInstance] timer:[NSString stringWithFormat:@"view_%@", self.viewID] timerInterval:updateInterval leeway:0.1 resumeType:EZTimerResumeTypeNow queue:EZTimerQueueTypeConcurrent queueName:@"update" repeats:YES action:^(NSString *timerName) {
