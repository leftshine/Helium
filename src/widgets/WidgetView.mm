...
- (void)reloadConfig {
    if (self.viewID) {
        ...
        // text config
        NSDictionary *colorDetails = [config valueForKey:@"colorDetails"] ? [config valueForKey:@"colorDetails"] : @{@"usesCustomColor" : @(NO)};
        ...
        UIFont *textFont = [FontUtils loadFontWithName:fontName size: fontSize bold: getBoolFromDictKey(config, @"textBold") italic: getBoolFromDictKey(config, @"textItalic")];
...
- (void)setTextAlignment:(NSInteger)align {
    self.labelView.textAlignment = align;
    self.maskLabelView.textAlignment = align;
}
